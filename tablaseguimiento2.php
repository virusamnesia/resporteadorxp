<?php 
	...
	$fechac=substr($fecha,8,2)."/".substr($fecha,5,2)."/".substr($fecha,0,4);
 ...
 ?>
 	<h4>FECHA DE CONSULTA: <?php echo $fechac; ?></h4>
	<div class="row">
		<div class="col-sm-12">
			...
			<table class="table table-hover table-condensed table-bordered " id="tablasegload">
				...
				<thead>
					<tr>
						<?php 
						echo " <td width='95px'  bgcolor='#DCF2F4'>EDITAR MINIMO</td>";
						...
						echo " <td width='95px'  bgcolor='#DCF2F4'>FAMILIA</td>";
						echo " <td width='95px'  bgcolor='#DCF2F4'>ESTATUS COMPRA</td>";
						...
						if($cneg=="true"){echo " <td width='100px'  bgcolor='#DCF2F4'>NEGADOS/ EXISTENCIAS</td>";}
						if($cminc=="true"){echo "<td bgcolor='#DCF2F4'>MINIMO COMPRA</td>";}
						if($csug=="true"){echo "<td bgcolor='#DCF2F4'>COMPRA SUGUERIDA</td>";}
						echo "<td bgcolor='#DCF2F4'>MES INVENTARIO</td>";
						$regs=0;
						$sumexist=0;
						$sumreq=0;
						$sumord=0;
				...
				<?php 
					...
					$querymar="SELECT ART, DESCU, PROV, COSTO, UNI, FACTOR, FAMILIA, ESTATUS_COM, ALM1, ALM2, ALM3, EXIST, REQ, ORD, DISPO, MINIMO, COT, PED, REM, OLA, NECES, COMPRA, NEG, MIC, C_SUG FROM SEG_INV_ALYC WHERE (COT > 0 OR MINIMO > 0) AND FECHA='$fechac' ".$condicion." ORDER BY ART";
					$resmar=sqlsrv_query($coni,$querymar);
					if(!$resmar){
					...
					while($rowmar=sqlsrv_fetch_object($resmar)){
						...
						$duni=$rowmar->UNI;
						$dfact=$rowmar->FACTOR;
						$dsta=$rowmar->ESTATUS_COM;
				 ?>
				 
				<tr>
					
					<td width="95px"> <button class="btn btn-warning glyphicon glyphicon-pencil" data-toggle="modal" data-target="#uminimo" onclick="stockmin('<?php echo $dart; ?>',<?php echo $dmin; ?>,<?php echo $dfact; ?>)"></button></td>
					<?php 
					echo "<td width='95px'>$rowmar->ART</td>";
					echo "<td width='130px'>$rowmar->DESCU</td>"; $regs=$regs+1;
					echo "<td width='140px'> $rowmar->PROV</td>";
					echo "<td width='95px'>$rowmar->FAMILIA</td>";
					?>
					<td width="95px"><?php echo $rowmar->ESTATUS_COM; ?>  <button class="btn btn-warning glyphicon glyphicon-edit" data-toggle="modal" data-target="#statuscom" onclick="statuscomp('<?php echo $dart; ?>','<?php echo $duni; ?>','<?php echo $dsta; ?>')"></button></td>
					<?php
					...
					if ($rowmar->MINIMO==0){echo "<td width='100px'> 0% </td>";} else{echo "<td width='100px'> ".ROUND($rowmar->NEG/$rowmar->MINIMO,2)."% </td>";}
					 ?>
					<!--
					<td>
				...
				<tr>
					...
					echo " <td width='100px'  bgcolor='#94DCE3'>  </td>";
					 ?>
				</tr>
				</tfoot>
				</tbody>
